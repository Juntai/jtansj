...
//	private static List<String> NaturesRemove = new ArrayList<String>();
	...
	// 添加停用词
	public static void insertStopWords(String[] sws){
...
//	}
	// 删除停用词，提取指定词性或删除指定词性
	// 注意：提取词性和删除词性只能一个有效
	public static List<Term> modifyResult(List<Term> all, String[] nis, String[] nrs){
		List<Term> result = new ArrayList<Term>();
		List<String> NaturesInclude = new ArrayList<String>();
		List<String> NaturesRemove = new ArrayList<String>();
		if(nis.length > 0){
			NaturesInclude = Arrays.asList(nis);
		}else if(nrs.length > 0){
			NaturesRemove = Arrays.asList(nrs);
		}
